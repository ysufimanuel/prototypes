...
(function () {
  ...
  function startRealtimeListeners() {
    ...
    if (!window.getActiveChurchId || !window.getActiveChurchId()) {
      console.warn(
        "[RTFB] churchId belum di-set — real-time listeners dilewati",
      );
      return;
    }
  ...
  async function _checkBirthdays(members) {
    ...
    console.log(
      `[RTFB] Ditemukan ${birthdayMembers.length} ulang tahun hari ini`,
    );
  ...
  function _scheduleDailyBirthdayCheck() {
    ...
    console.log(
      `[RTFB] Cek ulang tahun berikutnya: ${next.toLocaleString("id-ID")}`,
    );
  }
  ...
  async function init() {
    ...
    try {
      ...
      console.error(
        "[RTFB] app.js functions tidak ditemukan dalam 10 detik:",
        e,
      );
      return;
    }
    ...
    try {
      ...
      console.log(
        "[RTFB] Tidak ada session aktif — listeners akan dimulai setelah login",
      );
    }
    ...
    console.log(
      "[RTFB] ✅ Patch layer aktif — real-time & birthday notification siap",
    );
  }
