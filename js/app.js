...
async function initDataCache() {
  ...
  if (!isFirebaseReady()) {
    console.log(
      "[APP] Firebase not ready after waiting, using localStorage fallback",
    );
...
async function saveData(data) {
  ...
  try {
    ...
    showToast(
      currentLanguage === "id" ? "Gagal menyimpan data" : "Failed to save data",
      "error",
    );
    return false;
  }
        ...
        if (firebaseUser) {
          ...
          try {
            ...
            if (profile) {
              ...
              console.error(
                "[AUTH] Profil tidak ditemukan untuk uid:",
                firebaseUser.uid,
              );
            }
          } catch (err) {
...
async function _showMainApp(user) {
  ...
  document.getElementById("user-role-display").textContent = getRoleLabel(
    user.role,
  );
  ...
  if (loginForm) {
    loginForm.addEventListener("submit", async function (e) {
      ...
      notification.textContent =
        currentLanguage === "id" ? "Sedang memproses..." : "Processing...";
      ...
      if (result.success) {
        ...
        try {
          if (result.user && result.user.uid && window.getUserProfile) {
            ...
            if (profile && profile.churchId) {
              ...
              console.error(
                "[LOGIN] Profil tidak memiliki churchId, logout...",
              );
              await window.logoutFromFirebase();
              notification.textContent =
                currentLanguage === "id"
                  ? "Data user belum lengkap. Hubungi admin."
                  : "User data incomplete. Contact admin.";
        ...
        notification.textContent =
          currentLanguage === "id" ? "Login berhasil!" : "Login successful!";
...
function initDashboard() {
  ...
  document.getElementById("stat-total-members").textContent = (
    data.members || []
  ).filter((m) => m.status === "aktif").length;
  ...
  document.getElementById("stat-volunteers").textContent = (
    data.volunteers || []
  ).filter((v) => v.status === "aktif").length;
  ...
  if (churchId) {
    ...
    const el = document.getElementById("church-name-sidebar");
    if (el && data.churchProfile)
      el.textContent = data.churchProfile.nama || "Gereja";
  }
...
function renderUpcomingEvents() {
  ...
  container.innerHTML =
    upcoming
      ...
      .join("") ||
    `<p class="text-center" style="padding: 20px; color: var(--text-muted);">${getLang("no-upcoming-events") || "Tidak ada event mendatang"}</p>`;
}
...
function initAttendance() {
  ...
  if (eventSelect) {
    eventSelect.innerHTML =
      `<option value="">${getLang("select-event")}</option>` +
      data.events
        .filter((e) => e.status === "upcoming" || e.status === "ongoing")
        .map(
          (e) =>
            `<option value="${e.id}">${e.nama} - ${formatDate(e.start)}</option>`,
        )
        .join("");
  }
  ...
  if (filterEventSelect) {
    filterEventSelect.innerHTML =
      `<option value="">${getLang("all-events")}</option>` +
      data.events
        .map((e) => `<option value="${e.id}">${e.nama}</option>`)
        .join("");
  }
...
function showAttendanceTab(tab, clickedBtn) {
  document
    .querySelectorAll(".attendance-tab-content")
    .forEach((c) => c.classList.remove("active"));
  document
    .querySelectorAll(".attendance-tabs .tab-btn")
...
function renderTodayCheckin() {
  ...
  container.innerHTML =
    todayAttendance
      ...
      .join("") ||
    `<p class="text-center" style="padding: 20px; color: var(--text-muted);">${currentLanguage === "id" ? "Belum ada check-in hari ini" : "No check-ins today"}</p>`;
}
...
function renderAttendanceTable() {
  ...
  const eventFilter =
    document.getElementById("attendance-filter-event")?.value || "";
  const dateFilter =
  ...
  container.innerHTML =
    sorted
      ...
      .join("") ||
    `<tr><td colspan="5" class="text-center">${getLang("no-data") || "Tidak ada data"}</td></tr>`;
}
...
function checkInMember() {
  if (isViewOnly()) {
    showToast(
      currentLanguage === "id"
        ? "Anda tidak memiliki izin untuk check-in"
  ...
  if (!eventId || !memberName) {
    showToast(
      currentLanguage === "id"
        ? "Pilih event dan masukkan nama member"
        : "Select event and enter member name",
      "error",
    );
    return;
  }
  ...
  const member = data.members.find(
    (m) => m.nama.toLowerCase() === memberName.toLowerCase(),
  );

  if (!member) {
    showToast(
      currentLanguage === "id" ? "Member tidak ditemukan" : "Member not found",
      "error",
    );
    return;
  }
  ...
  showToast(
    `${member.nama} ${currentLanguage === "id" ? "berhasil check-in" : "checked in successfully"}`,
    "success",
  );
}
...
function initDonations() {
  ...
  const addButton = document.querySelector(
    "#page-donations .action-buttons .btn-primary",
  );
  if (addButton) {
...
function updateDonationSummary() {
  ...
  document.getElementById("donation-average").textContent =
...
function renderDonationsTable() {
  ...
  let filtered = data.donations.filter((d) => {
    const matchType =
      !typeFilter ||
      d.tipe === typeFilter ||
      (typeFilter === "lainnya" && d.tipe === "lainnya");
    const matchMonth = !monthFilter || d.tanggal.startsWith(monthFilter);
    return matchType && matchMonth;
  });
  ...
  container.innerHTML =
    filtered
      .map((d) => {
        ...
        const tipeDisplay =
          d.tipe === "lainnya" && d.customType ? d.customType : d.tipe;
      ...
      .join("") ||
    `<tr><td colspan="6" class="text-center">${getLang("no-data") || "Tidak ada data"}</td></tr>`;
}
...
function showAddDonationModal() {
  ...
  document.getElementById("modal-donation-title").textContent =
    getLang("add-donation");
...
function editDonation(id) {
  ...
  if (donation) {
    ...
    document.getElementById("modal-donation-title").textContent =
      currentLanguage === "id" ? "Edit Donasi" : "Edit Donation";
    ...
    document.getElementById("donation-keterangan").value =
      donation.keterangan || "";

    // Handle custom type
    if (donation.tipe === "lainnya" && donation.customType) {
      document.getElementById("donation-custom-type-container").style.display =
        "block";
...
function saveDonation(e) {
  ...
  if (isViewOnly()) {
    showToast(
      currentLanguage === "id"
        ? "Anda tidak memiliki izin untuk menyimpan data"
  ...
  if (donationTipe === "lainnya" && !customType) {
    showToast(
      currentLanguage === "id"
        ? "Tipe donasi lainnya wajib diisi"
        : "Custom donation type is required",
      "error",
    );
    return;
  }
  ...
  if (editingDonationId) {
    ...
    const tipeDisplay =
      donationTipe === "lainnya" && customType ? customType : donationTipe;
    if (!data.activities) data.activities = [];
    data.activities.unshift({
      id: Date.now(),
      type: "donation",
      action:
        currentLanguage === "id" ? "Donasi diterima" : "Donation received",
...
function populateDonorSelect(selectedId = null) {
  ...
  select.innerHTML =
    `<option value="">${getLang("select-donor")}</option>` +
    data.members
...
function renderEventsGrid() {
  ...
  const searchTerm =
    document.getElementById("search-events")?.value?.toLowerCase() || "";
  const statusFilter =
  ...
  container.innerHTML =
    filtered
      .map((e) => {
        ...
        const eventType =
          e.tipe === "lainnya" && e.customType ? e.customType : e.tipe;
...
function handleDonationTypeChange() {
  ...
  const customTypeContainer = document.getElementById(
    "donation-custom-type-container",
  );
...
function handleVolunteerAreaChange() {
  ...
  const customAreaContainer = document.getElementById(
    "volunteer-custom-area-container",
  );
...
function editEvent(id) {
  ...
  if (event) {
    editingEventId = id;
    document.getElementById("modal-event-title").textContent =
      currentLanguage === "id" ? "Edit Event" : "Edit Event";
    ...
    if (event.tipe === "lainnya" && event.customType) {
      document.getElementById("event-custom-type-container").style.display =
        "block";
  ...
  container.innerHTML =
    event.participants
      .map((p, i) => {
        ...
        const deleteButton = isViewOnly()
          ? ""
          : `<button class="btn-action btn-delete" onclick="removeParticipant(${i})"><i class="fas fa-trash"></i></button>`;
...
function addParticipant() {
  if (isViewOnly()) {
    showToast(
      currentLanguage === "id"
        ? "Anda tidak memiliki izin untuk menambah peserta"
...
function saveManualParticipant(e) {
  ...
  if (!event) {
    showToast(
      currentLanguage === "id" ? "Event tidak ditemukan" : "Event not found",
      "error",
    );
    return;
  }
...
function removeParticipant(index) {
  if (isViewOnly()) {
    showToast(
      currentLanguage === "id"
        ? "Anda tidak memiliki izin untuk menghapus peserta"
...
function initVolunteers() {
  ...
  // Hide add button for view-only users
  const addButton = document.querySelector(
    "#page-volunteers .action-buttons .btn-primary",
  );
  if (addButton) {
    addButton.style.display = isViewOnly() ? "none" : "inline-flex";
  }
...
function renderVolunteersGrid() {
  ...
  const searchTerm =
    document.getElementById("search-volunteers")?.value?.toLowerCase() || "";
  ...
  const areaLabels = {
    ...
    singer: "Singer",
    "worship-leader":
      currentLanguage === "id" ? "Worship Leader" : "Worship Leader",
    "pemain-musik": currentLanguage === "id" ? "Pemain Musik" : "Musician",
    penari: currentLanguage === "id" ? "Penari" : "Dancer",
    kolektan:
      currentLanguage === "id" ? "Kolektan/Perpuluhan" : "Collection/Tithe",
  ...
  container.innerHTML =
    filtered
      .map((v) => {
        ...
        const areaDisplay =
          v.area === "lainnya" && v.customArea
            ? v.customArea
            : areaLabels[v.area] || v.area;
...
function showAddVolunteerModal() {
  ...
  document.getElementById("modal-volunteer-title").textContent =
    getLang("add-volunteer");
  ...
  document
    .querySelectorAll('input[name="jadwal"]')
...
function editVolunteer(id) {
  ...
  if (volunteer) {
    ...
    document.getElementById("modal-volunteer-title").textContent =
      currentLanguage === "id" ? "Edit Relawan" : "Edit Volunteer";
    ...
    if (volunteer.area === "lainnya" && volunteer.customArea) {
      document.getElementById("volunteer-custom-area-container").style.display =
        "block";
      document.getElementById("volunteer-custom-area").value =
        volunteer.customArea;
    ...
    if (volunteer.memberId) {
      ...
      document.getElementById("volunteer-member-section").style.display =
        "block";
      document.getElementById("volunteer-external-section").style.display =
        "none";
      ...
      document.getElementById("volunteer-external-section").style.display =
        "block";
      document.getElementById("volunteer-external-nama").value =
        volunteer.externalNama || "";
      document.getElementById("volunteer-external-email").value =
        volunteer.externalEmail || "";
      document.getElementById("volunteer-external-telepon").value =
        volunteer.externalTelepon || "";
    }
    ...
    if (volunteer.jadwal && Array.isArray(volunteer.jadwal)) {
      volunteer.jadwal.forEach((schedule) => {
        const checkbox = document.querySelector(
          `input[name="jadwal"][value="${schedule}"]`,
        );
    ...
    document.getElementById("volunteer-service-date").value =
      volunteer.serviceDate || "";
    document.getElementById("volunteer-service-time").value =
...
function saveVolunteer(e) {
  ...
  if (volunteerArea === "lainnya" && !customArea) {
    showToast(
      currentLanguage === "id"
        ? "Area pelayanan lainnya wajib diisi"
        : "Custom service area is required",
      "error",
    );
    return;
  }
  ...
  if (source === "member") {
    volunteerData.memberId = parseInt(
    ...
    volunteerData.externalNama = document.getElementById(
      "volunteer-external-nama",
    ).value;
    volunteerData.externalEmail = document.getElementById(
      "volunteer-external-email",
    ).value;
    volunteerData.externalTelepon = document.getElementById(
      "volunteer-external-telepon",
    ).value;
  }
...
function populateVolunteerMemberSelect(selectedId = null) {
  ...
  select.innerHTML =
    `<option value="">${getLang("select-member")}</option>` +
    data.members
      .filter((m) => m.status === "aktif")
      .map(
        (m) =>
...
function showAssignments(volunteerId) {
  ...
  // Display custom area if "lainnya" is selected
  const areaDisplay =
    volunteer.area === "lainnya" && volunteer.customArea
      ? volunteer.customArea
      : areaLabels[volunteer.area] || volunteer.area;

  document.getElementById("detail-title").textContent =
    `${getLang("assignments") || "Tugas"}: ${nama}`;
  // Format service date and time for display
  const serviceDateDisplay = volunteer.serviceDate
    ? formatDate(volunteer.serviceDate)
    : null;
            ...
            ${
              assignments.length > 0
                ...
                `;
                    ...
                    .join("")
                : `<p style="color: var(--text-muted); text-align: center; padding: 20px;">${currentLanguage === "id" ? "Belum ada tugas yang ditugaskan" : "No assignments yet"}</p>`
            }
        </div>
    `;
...
function renderMembersTable() {
  ...
  const searchTerm =
    document.getElementById("search-members")?.value?.toLowerCase() || "";
  ...
  container.innerHTML =
    filtered
      ...
      .join("") ||
    `<tr><td colspan="${isUser ? 6 : 8}" class="text-center">${getLang("no-data") || "Tidak ada data"}</td></tr>`;

  // Update action bar visibility
  const addButton = document.querySelector(
    "#page-members .action-buttons .btn-primary",
  );
  if (addButton) {
...
function showAddMemberModal() {
  ...
  document.getElementById("modal-member-title").textContent =
    currentLanguage === "id" ? "Tambah Member Baru" : "Add New Member";
  document.getElementById("form-member").reset();
  document.getElementById("member-id").value = "";

  // Set default join date to today
  document.getElementById("member-join-date").value = new Date()
    .toISOString()
    .split("T")[0];
...
function editMember(id) {
  ...
  document.getElementById("member-tempat-lahir").value =
    member.tempatLahir || "";
...
function viewMemberDetail(id) {
  ...
  document.getElementById("detail-title").textContent =
    currentLanguage === "id" ? "Detail Member" : "Member Detail";
  document.getElementById("detail-content").innerHTML = `
        <div class="detail-section">
...
function populateGroupSelect(selectedId = null) {
  ...
  data.groups.forEach((g) => {
    const isSelected =
      selectedGroupId && g.id === selectedGroupId ? "selected" : "";
...
function renderFamiliesGrid() {
  ...
  const searchTerm =
    document.getElementById("search-families")?.value?.toLowerCase() || "";
  ...
  // Hide add button for view-only users
  const addButton = document.querySelector(
    "#page-families .action-buttons .btn-primary",
  );
  if (addButton) {
    addButton.style.display = isViewOnly() ? "none" : "inline-flex";
  }
...
function showAddFamilyModal() {
  ...
  document.getElementById("modal-family-title").textContent =
    getLang("add-family");
...
function editFamily(id) {
  ...
  if (family) {
    editingFamilyId = id;
    document.getElementById("modal-family-title").textContent =
      currentLanguage === "id" ? "Edit Keluarga" : "Edit Family";
...
function saveFamily(e) {
  ...
  const selectedMembers = Array.from(
    document.querySelectorAll("#family-members input:checked"),
...
function viewFamilyDetail(id) {
  ...
  const anggota = family.anggota
    .map((mid) => data.members.find((m) => m.id === mid))
    .filter(Boolean);

  document.getElementById("detail-title").textContent =
    currentLanguage === "id" ? "Detail Keluarga" : "Family Detail";
  document.getElementById("detail-content").innerHTML = `
        <div class="detail-section">
...
function populateFamilyMemberSelects(
  selectedKepala = null,
  selectedAnggota = [],
) {
  const data = getData();
...
function renderGroupsGrid() {
  ...
  // Hide add button for view-only users
  const addButton = document.querySelector(
    "#page-groups .action-buttons .btn-primary",
  );
  if (addButton) {
    addButton.style.display = isUser ? "none" : "inline-flex";
  }
...
function showAddGroupModal() {
  ...
  editingGroupId = null;
  document.getElementById("modal-group-title").textContent =
    getLang("add-group");
...
function editGroup(id) {
  ...
  if (group) {
    editingGroupId = id;
    document.getElementById("modal-group-title").textContent =
      currentLanguage === "id" ? "Edit Group" : "Edit Group";
    ...
    document.getElementById("group-show-phone").checked =
      group.showPhone !== false;
...
function manageGroupMembers(groupId) {
  ...
  // Update modal title
  document.getElementById("modal-group-members-title").textContent =
    `${currentLanguage === "id" ? "Kelola Anggota" : "Manage Members"}: ${group.nama}`;
...
function renderGroupMembersList() {
  ...
  group.anggota.forEach((memberId) => {
    ...
    memberItem.style.cssText =
      "display: flex; align-items: center; gap: 12px; padding: 12px; background: var(--dark-bg); border-radius: var(--radius-sm); margin-bottom: 8px;";
...
function populateAvailableMembersSelect() {
  ...
  const availableMembers = data.members.filter(
    (m) => m.status === "aktif" && !group.anggota.includes(m.id),
  );
...
function addMemberToGroup() {
  if (isViewOnly()) {
    showToast(
      currentLanguage === "id"
        ? "Anda tidak memiliki izin untuk menambah anggota"
  ...
  if (group) {
    if (!group.anggota.includes(memberId)) {
      ...
      showToast(
        currentLanguage === "id"
          ? "Anggota berhasil ditambahkan ke group"
      ...
      showToast(
        currentLanguage === "id"
          ? "Member sudah ada dalam group ini"
...
function removeMemberFromGroup(memberId) {
  if (isViewOnly()) {
    showToast(
      currentLanguage === "id"
        ? "Anda tidak memiliki izin untuk menghapus anggota"
  ...
  if (
    !confirm(
      currentLanguage === "id"
        ? "Apakah Anda yakin ingin menghapus anggota ini dari group?"
  ...
  if (group) {
    ...
    if (index > -1) {
      ...
      showToast(
        currentLanguage === "id"
          ? "Anggota berhasil dihapus dari group"
      ...
      showToast(
        currentLanguage === "id"
          ? "Anggota tidak ditemukan dalam group"
...
function showAddAssignmentModal() {
  if (isViewOnly()) {
    showToast(
      currentLanguage === "id"
        ? "Anda tidak memiliki izin untuk menambah penugasan"
...
function editAssignment(id) {
  ...
  if (assignment) {
    ...
    document.getElementById("assignment-catatan").value =
      assignment.catatan || "";
    openModal("modal-assignment");
  }
}
...
function saveAssignment(e) {
  ...
  const assignmentData = {
    volunteerId: parseInt(
      document.getElementById("assignment-volunteer").value,
    ),
...
function populateAssignmentVolunteerSelect(selectedId = null) {
  ...
  select.innerHTML =
    `<option value="">${getLang("select-volunteer")}</option>` +
    data.volunteers
...
function showVolunteersTab(tab, clickedBtn) {
  document
    .querySelectorAll(".volunteers-tab-content")
    .forEach((c) => c.classList.remove("active"));
  document
    .querySelectorAll(".volunteers-tabs .tab-btn")
    .forEach((b) => b.classList.remove("active"));
...
function showCommunicationTab(tab, clickedBtn) {
  document
    .querySelectorAll(".communication-tab-content")
    .forEach((c) => c.classList.remove("active"));
  document
    .querySelectorAll(".communication-tabs .tab-btn")
    .forEach((b) => b.classList.remove("active"));
...
function renderAnnouncementsList() {
  ...
  container.innerHTML =
    sorted
      .map((a) => {
        const deleteButton = isViewOnly()
          ? ""
          : `<button class="btn-action btn-delete" onclick="deleteAnnouncement(${a.id})"><i class="fas fa-trash"></i></button>`;
  ...
  // Hide add button for view-only users
  const addButton = document.querySelector(
    "#communication-announcements .action-buttons .btn-primary",
  );
  if (addButton) {
    addButton.style.display = isViewOnly() ? "none" : "inline-flex";
  }
...
function showAddAnnouncementModal() {
  if (isViewOnly()) {
    showToast(
      currentLanguage === "id"
        ? "Anda tidak memiliki izin untuk menambah pengumuman"
...
function sendBroadcast() {
  if (isViewOnly()) {
    showToast(
      currentLanguage === "id"
        ? "Anda tidak memiliki izin untuk mengirim broadcast"
...
function saveBroadcastDraft() {
  showToast(
    currentLanguage === "id" ? "Draft disimpan" : "Draft saved",
    "success",
  );
}
...
function refreshContacts() {
  renderContactsList();
  showToast(
    currentLanguage === "id" ? "Kontak diperbarui" : "Contacts updated",
    "success",
  );
}
...
function generateReport(type) {
  ...
  switch (type) {
    case "members":
      title.textContent =
        currentLanguage === "id" ? "Laporan Jemaat" : "Members Report";
      reportHTML = generateMembersReport(data);
      break;
    case "attendance":
      title.textContent =
        currentLanguage === "id" ? "Laporan Kehadiran" : "Attendance Report";
      reportHTML = generateAttendanceReport(data);
      break;
    case "donations":
      title.textContent =
        currentLanguage === "id" ? "Laporan Donasi" : "Donations Report";
      reportHTML = generateDonationsReport(data);
      break;
    case "system":
      title.textContent =
        currentLanguage === "id" ? "Statistik Sistem" : "System Statistics";
...
function generateMembersReport(data) {
  ...
  const inactiveMembers = data.members.filter(
...
function generateAttendanceReport(data) {
  ...
  const averagePerEvent =
    uniqueEvents > 0 ? Math.round(totalAttendance / uniqueEvents) : 0;
...
function generateDonationsReport(data) {
  ...
  const average =
    data.donations.length > 0 ? Math.round(total / data.donations.length) : 0;
...
function saveReportAs(format) {
  ...
  let reportType = "";
  if (title.includes("Jemaat") || title.includes("Members"))
    reportType = "members";
  else if (title.includes("Kehadiran") || title.includes("Attendance"))
    reportType = "attendance";
  else if (title.includes("Donasi") || title.includes("Donations"))
...
function editUser(id) {
  ...
  // Kalau edit diri sendiri, role tidak boleh diubah
...
async function saveUser(e) {
  ...
  if (!canManageUsers()) {
    showToast(
      typeof t === "function" ? t("accessDenied") : "Akses ditolak",
      "error",
    );
    return;
  }
...
function formatTimeAgo(timestamp) {
  ...
  if (diff < 3600)
    return `${Math.floor(diff / 60)} ${currentLanguage === "id" ? "menit" : "min"} ${currentLanguage === "id" ? "lalu" : "ago"}`;
  if (diff < 86400)
    return `${Math.floor(diff / 3600)} ${currentLanguage === "id" ? "jam" : "hr"} ${currentLanguage === "id" ? "lalu" : "ago"}`;
  ...
  for (let i = 0; i < labelNames.length; i++) {
    ...
    if (count >= 1) {
      if (currentLanguage === "id") {
        ...
        return count === 1
          ? `${count} ${labelNames[i]} ago`
          : `${count} ${labelNames[i]}s ago`;
      }
    }
  }
...
function updateFinanceSummary() {
  ...
  const saldo =
    (data.finance?.saldoAwal || 0) + totalPemasukan - totalPengeluaran;

  // Update display
  document.getElementById("finance-saldo").textContent = formatRupiah(saldo);
  document.getElementById("finance-total-pemasukan").textContent =
    formatRupiah(totalPemasukan);
  document.getElementById("finance-total-pengeluaran").textContent =
...
function updateFinanceChart() {
  ...
  const data = getData();
  const period =
    document.getElementById("finance-chart-period")?.value || "month";
  ...
  if (period === "month") {
    ...
    (data.pemasukan || [])
      .filter(
        (p) => p.status === "approved" && p.tanggal.startsWith(currentMonth),
      )
      .forEach((p) => {
    ...
    (data.pemasukan || [])
      .filter(
        (p) =>
          p.status === "approved" && p.tanggal.startsWith(String(currentYear)),
      )
      .forEach((p) => {
...
function updateIncomeCategoryChart() {
  ...
  const data = getData();
  const kategoriPemasukan = (data.financeCategories || []).filter(
    (k) => k.tipe === "pemasukan",
  );
  ...
  (data.pemasukan || [])
    ...
    .forEach((p) => {
      ...
      if (kategori) {
        categoryTotals[kategori.nama] =
          (categoryTotals[kategori.nama] || 0) + p.jumlah;
      }
    });
...
function updateExpenseCategoryChart() {
  ...
  const data = getData();
  const kategoriPengeluaran = (data.financeCategories || []).filter(
...
function populateKategoriSelects() {
  ...
  const filterPemasukanSelect = document.getElementById(
    "filter-pemasukan-kategori",
  );
  const filterPengeluaranSelect = document.getElementById(
    "filter-pengeluaran-kategori",
  );

  if (pemasukanSelect) {
    pemasukanSelect.innerHTML =
      '<option value="">' + getLang("select-category") + "</option>";
  ...
  if (filterPemasukanSelect) {
    filterPemasukanSelect.innerHTML =
      '<option value="">' + getLang("all-categories") + "</option>";
    (data.financeCategories || [])
      .filter((k) => k.tipe === "pemasukan")
      .forEach((k) => {
  ...
  if (filterPengeluaranSelect) {
    filterPengeluaranSelect.innerHTML =
      '<option value="">' + getLang("all-categories") + "</option>";
    (data.financeCategories || [])
      .filter((k) => k.tipe === "pengeluaran")
      .forEach((k) => {
...
function renderPemasukan() {
  ...
  const searchTerm =
    document.getElementById("search-pemasukan")?.value?.toLowerCase() || "";
  const kategoriFilter =
    document.getElementById("filter-pemasukan-kategori")?.value || "";
  const monthFilter =
    document.getElementById("filter-pemasukan-month")?.value || "";
  ...
  tbody.innerHTML = filtered
    .map((p) => {
      const kategori = data.financeCategories?.find(
        (k) => k.id === p.kategoriId,
      );
      const donatur = data.donatur?.find((d) => d.id === p.donaturId);
      ...
      if (p.status === "approved") {
        ...
        statusBadge =
          '<span class="badge badge-danger">' + getLang("rejected") + "</span>";
      } else {
        statusBadge =
          '<span class="badge badge-warning">' + getLang("pending") + "</span>";
      }

      const canEdit =
        !isViewOnly() && (isSuperAdmin() || p.status !== "approved");
...
function renderPengeluaran() {
  ...
  const searchTerm =
    document.getElementById("search-pengeluaran")?.value?.toLowerCase() || "";
  const kategoriFilter =
    document.getElementById("filter-pengeluaran-kategori")?.value || "";
  const monthFilter =
    document.getElementById("filter-pengeluaran-month")?.value || "";
...
function renderKategori() {
  ...
  const searchTerm =
    document.getElementById("search-kategori")?.value?.toLowerCase() || "";
...
function renderApprovalTab() {
  ...
  // Pending items
  const pendingPemasukan = (data.pemasukan || []).filter(
    (p) => p.status === "pending",
  );
  const pendingPengeluaran = (data.pengeluaran || []).filter(
    (p) => p.status === "pending",
  );
  ...
  historyList.innerHTML = historyItems
    .map((h) => {
      ...
      const kategori = item
        ? data.financeCategories?.find((k) => k.id === item.kategoriId)
        : null;
...
function showAddPemasukanModal() {
  ...
  document.getElementById("pemasukan-tanggal").value = new Date()
    .toISOString()
    .split("T")[0];
  document.getElementById("modal-pemasukan-title").textContent =
    getLang("add-income");
...
function showAddPengeluaranModal() {
  ...
  document.getElementById("pengeluaran-tanggal").value = new Date()
    .toISOString()
    .split("T")[0];
  document.getElementById("modal-pengeluaran-title").textContent =
    getLang("add-expense");
...
function showAddKategoriModal() {
  ...
  document.getElementById("modal-kategori-title").textContent =
    getLang("add-category");
...
function editPemasukan(id) {
  ...
  if (isViewOnly()) {
    showToast(
      "Anda hanya dapat melihat data. Hubungi admin untuk mengubah data.",
      "error",
    );
    return;
  }
...
function savePemasukan() {
  ...
  if (kategoriId === "other") {
    const customKategori = document
      .getElementById("pemasukan-kategori-lainnya")
      .value.trim();
    if (!customKategori) {
    ...
    const newKategoriId =
      data.financeCategories.length > 0
        ? Math.max(...data.financeCategories.map((k) => k.id)) + 1
        : 1;
  ...
  if (id) {
    ...
    const newId =
      data.pemasukan.length > 0
        ? Math.max(...data.pemasukan.map((p) => p.id)) + 1
        : 1;
    data.pemasukan.push({
      id: newId,
      kategoriId,
...
function savePengeluaran() {
  ...
  if (kategoriId === "other") {
    const customKategori = document
      .getElementById("pengeluaran-kategori-lainnya")
      .value.trim();
    if (!customKategori) {
      showToast("Mohon masukkan nama kategori", "error");
      return;
  ...
  if (id) {
    ...
    const newId =
      data.pengeluaran.length > 0
        ? Math.max(...data.pengeluaran.map((p) => p.id)) + 1
        : 1;
...
async function approveItem(tipe, id) {
  ...
  if (!item) {
    showToast(
      currentLanguage === "id" ? "Data tidak ditemukan" : "Data not found",
      "error",
    );
    return;
  }
...
function updateLaporanView() {
  ...
  document.getElementById("laporan-date-filter").style.display =
    type === "harian" ? "block" : "none";
  document.getElementById("laporan-month-filter").style.display =
    type === "bulanan" ? "block" : "none";
  document.getElementById("laporan-year-filter").style.display =
    type === "tahunan" ? "block" : "none";
  ...
  if (type === "harian") {
    ...
    pemasukanFiltered = (data.pemasukan || []).filter(
      (p) => p.status === "approved" && p.tanggal === date,
    );
    pengeluaranFiltered = (data.pengeluaran || []).filter(
      (p) => p.status === "approved" && p.tanggal === date,
    );
    ...
    pemasukanFiltered = (data.pemasukan || []).filter(
      (p) => p.status === "approved" && p.tanggal.startsWith(month),
    );
    pengeluaranFiltered = (data.pengeluaran || []).filter(
      (p) => p.status === "approved" && p.tanggal.startsWith(month),
    );
    ...
    pemasukanFiltered = (data.pemasukan || []).filter(
      (p) => p.status === "approved" && p.tanggal.startsWith(year),
    );
    pengeluaranFiltered = (data.pengeluaran || []).filter(
      (p) => p.status === "approved" && p.tanggal.startsWith(year),
    );
  ...
  const totalPemasukan = pemasukanFiltered.reduce(
    (sum, p) => sum + p.jumlah,
    0,
  );
  const totalPengeluaran = pengeluaranFiltered.reduce(
    (sum, p) => sum + p.jumlah,
    0,
  );
                    ...
                    <tbody>
                        ${
                          pemasukanFiltered
                            ...
                            .join("") ||
                          `<tr><td colspan="3" class="text-center">${getLang("none")}</td></tr>`
                        }
                    </tbody>
                </table>
            </div>
...
function exportFinanceData(type) {
  ...
  const kategoriFilter =
    document.getElementById(`filter-${type}-kategori`)?.value || "";
  const monthFilter =
...
function exportLaporan() {
  ...
  const exportData = [
    ...
    [
      ...
      "",
      "",
      pemasukanFiltered.reduce((sum, p) => sum + p.jumlah, 0),
    ],
    [],
    ["PENGELUARAN"],
    ["Tanggal", "Kategori", "Keterangan", "Jumlah"],
    ...
    [
      ...
      "",
      "",
      pengeluaranFiltered.reduce((sum, p) => sum + p.jumlah, 0),
    ],
    [],
    [
      "SALDO",
